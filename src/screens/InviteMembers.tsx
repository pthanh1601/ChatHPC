...
import { Search, X, Check, ChevronDown, CheckCircle, Circle } from 'lucide-react-native';
...
export function InviteMembers({ setScreen }: InviteMembersProps) {
  ...
  const [currentRoomMemberIds, setCurrentRoomMemberIds] = useState<string[]>([]);
    ...
    const fetchMembers = async () => {
      ...
      try {
        ...
        if (client) {
          ...
          console.log("Fetched existing members:", ids);
          setCurrentRoomMemberIds(ids);
        }
      } catch (e) {
        console.log("Lỗi lấy danh sách thành viên:", e);
      }
    };
    fetchMembers();
  }, []);
  ...
  const handleSearch = (term: string) => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setSearchResults(results.filter((u: any) => !currentRoomMemberIds.includes(u.user_id)));
      } catch (e) {
  ...
  const handleInviteSelected = async () => {
    ...
    try {
      ...
      if (roomId) {
        ...
        setTimeout(() => setScreen('room_details'), 1500);
      } else {
      ...
      <SafeAreaView className="bg-[#15191E]">
        <View className="pt-4 pb-2 px-4 flex-row items-center" style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight || 20 : 16 }}>
          ...
          {selectedUsers.length > 0 ? (
            ...
            <TouchableOpacity onPress={() => setScreen(currentActiveRoomId ? 'room_details' : 'chat_list')}>
              ...
              searchResults.map((user) => {
                ...
                return (
                  <View key={user.user_id} className="flex-row items-center justify-between mb-5">
                    <View className="flex-row items-center flex-1">
                      ...
                      <View className="ml-3 flex-1 mr-2">
                        <Text className="text-white text-[17px] font-semibold" numberOfLines={1}>
                          {displayName} <Text className="text-gray-400 font-normal">({user.user_id.split(':')[0]})</Text>
                        </Text>
                        {(() => {
                    ...
                    <TouchableOpacity onPress={() => toggleSelectUser(user.user_id)} className="p-2">
